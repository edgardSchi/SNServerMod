	...
	static void UpdateItemInfoLiquidType(Widget root_widget, EntityAI item)
	{
		...
		else if( item_base && item_base.GetQuantity() > 0 && item_base.IsLiquidContainer() )
		{
			...
			switch(liquid_type)
			{
				...
				{
					WidgetTrySetText(root_widget, "ItemLiquidTypeWidget", "#inv_inspect_gasoline", Colors.COLOR_BADLY_DAMAGED);
					break;
				}
				
				case LIQUID_DIESEL:
				{
					WidgetTrySetText(root_widget, "ItemLiquidTypeWidget", "#inv_inspect_diesel", Colors.COLOR_BADLY_DAMAGED);
					break;
				}
				...
				case LIQUID_SODA:
				{
					WidgetTrySetText(root_widget, "ItemLiquidTypeWidget", "Soda", Colors.COLOR_SODA);
					break;
				}
				default:
				{
